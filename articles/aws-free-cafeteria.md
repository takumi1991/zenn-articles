...
## デモサイト
実際にAWSのAlways Freeのサービス群を活用して構築した、AWS Cloud Practitionerの模擬試験アプリです。S3は常時無料枠ではないため使わず、Lambdaでページを都度生成するサーバサイドレンダリング(SSR)となります。DynamoDB、CloudFront等も利用しています。
